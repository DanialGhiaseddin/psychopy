...
_localized.update({'sound': _translate('Sound'),
                   'tdt_config': _translate('TDT Config File'),
                   'volume': _translate('Volume'),
                   'syncScreenRefresh': _translate('Sync Start With Screen')})


class TDTSoundComponent(BaseComponent):
    """An event class for presenting sound stimuli"""
    categories = ['Stimuli']
    targets = ['PsychoPy', 'PsychoJS']
    iconFile = Path(__file__).parent / 'sound.png'
    tooltip = _translate('Sound: play recorded files or generated sounds, recorded file must be loaded in synapse '
                         'software', )

    def __init__(self, exp, parentName, name='tdt_sound_1', sound='A', volume=1,
                 startType='time (s)', startVal='0.0',
                 stopType='duration (s)', stopVal='1.0',
                 startEstim='', durationEstim='',
                 syncScreenRefresh=True):
        super(TDTSoundComponent, self).__init__(
            exp, parentName, name,
        ...
        self.exp.requirePsychopyLibs(['tdt_sound'])
        self.order += [
            "tdt_config",
            "sound",  # Basic tab
        ...
        self.params['tdt_config'] = Param(
            "Config.ymal", valType='str', inputType="file", allowedTypes=[], updates='constant', categ='Basic',
            label=_localized['tdt_config'])

        hnt = _translate("A sound can be a note name (e.g. A or Bf), a number"
                         " to specify Hz (e.g. 440) or a filename")
        self.params['sound'] = Param(
            sound, valType='str', inputType="file", allowedTypes=[], updates='constant', categ='Playback',
    ...
    def writeInitCode(self, buff):
        ...
        if '$' in str(inits['stopVal'].val):
            ...
            if inits['stopVal'].val in ['', None, 'None']:
                ...
                inits['stopVal'].val = -1
        buff.writeIndented("%s = tdt_sound.TDTSound(None)\n" % inits['name'])
